# This script runs PGLS on connection length and cerebral volume.
...
mydata <- read.csv("../pgls/connectionLengthNormalized.csv")
...
mylambda <- "ML"
df1 <- tidyPgls(pgls(propLong~SupraTentorialVol, data=comp.data, lambda=mylambda)) # proportion long connections vs cerebral volume
df2 <- tidyPgls(pgls(propShort~SupraTentorialVol, data=comp.data, lambda=mylambda)) # proportion short connections vs cerebral volume
results = rbind(df1, df2)

write.csv(results, "../pgls/pglsConnectionLengthNormalized.csv")
