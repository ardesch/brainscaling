...
mylambda <- "ML"
df1 <- tidyPgls(pgls(conn_asymmetry~SupraTentorialVol, data=comp.data, lambda=mylambda)) # weighted distance vs supratentorial volume
df2 <- tidyPgls(pgls(conn_asymmetry~cort_surf, data=comp.data, lambda=mylambda)) # weighted distance vs cortical surface area volume
